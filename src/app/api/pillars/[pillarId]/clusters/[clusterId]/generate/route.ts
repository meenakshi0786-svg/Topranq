...
export async function POST(
  ...
  // Check article count limit (admins have no limit)
  if (!isAdmin(user.email)) {
    ...
    let articleCount = 0;
    for (const dId of userDomainIds) {
      const count = db.select().from(schema.articles).where(eq(schema.articles.domainId, dId)).all().length;
      articleCount += count;
    }

    if (articleCount >= maxArticles) {
      return NextResponse.json(
        { error: `You've reached your limit of ${maxArticles} articles. Purchase another pack to generate more.` },
        { status: 403 }
      );
    }
  }
