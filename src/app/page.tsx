      ...
      {showSignIn && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.5)" }}>
          <div className="w-full max-w-md mx-4 rounded-2xl overflow-hidden" style={{ background: "var(--bg-white)", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.25)" }}>
            {/* Header */}
            <div className="p-7 text-center" style={{ background: "linear-gradient(135deg, #4F6EF7, #7C5CFC)" }}>
              ...
              <h2 className="text-xl font-bold text-white mb-1">Ready to analyze</h2>
              <p className="text-sm" style={{ color: "rgba(255,255,255,0.85)" }}>{domainUrl}</p>
            </div>

            {/* Body */}
            <div className="p-6">
              <p className="text-sm text-center mb-5" style={{ color: "var(--text-secondary)" }}>
                Sign in with Google to run your SEO audit, generate blog articles, and get your review emails delivered.
              </p>

              {/* Google Sign In Button — primary CTA */}
              <button
                onClick={signInWithGoogle}
                className="w-full flex items-center justify-center gap-3 py-3.5 rounded-xl text-sm font-semibold cursor-pointer transition-all mb-4"
                style={{ background: "#4F6EF7", color: "#fff", border: "none" }}
                onMouseEnter={(e) => { e.currentTarget.style.background = "#3F5EE0"; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = "#4F6EF7"; }}
              >
                <svg width="18" height="18" viewBox="0 0 24 24" style={{ background: "#fff", borderRadius: 4, padding: 2 }}>
              ...
              <div className="p-4 rounded-xl mb-4" style={{ background: "var(--bg)" }}>
                <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "var(--text-muted)" }}>What happens next</p>
                <div className="space-y-2.5">
                  {[
                    { text: "We crawl up to 25 pages and run 47 SEO checks" },
                    { text: "AI suggests blog topics based on your content gaps" },
                    { text: "Review & approve articles via email before they're published" },
                  ].map((item, i) => (
                    <div key={item.text} className="flex items-start gap-2.5">
                      <div className="shrink-0 mt-0.5 w-4 h-4 rounded-full flex items-center justify-center" style={{ background: "var(--accent-light)", color: "var(--accent)", fontSize: 10, fontWeight: 700 }}>{i + 1}</div>
                      <span className="text-xs leading-relaxed" style={{ color: "var(--text-secondary)" }}>{item.text}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              >
                Continue without account (results won&apos;t be saved)
              </button>
            </div>
          </div>
        </div>
